...
def process_image_array(img_array):
    """Processes a BGR numpy array image to detect acne."""
    skin, original, meta = get_skin(img_array)
    if skin is None:
        return {"count": 0, "severity": 0, "status": "no_face"}

    result, count, score, mask = detect_acne(skin, original, meta)
    return {"count": count, "severity": score, "status": "success"}
